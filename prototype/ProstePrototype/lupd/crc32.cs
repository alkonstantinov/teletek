...
namespace lupd
{
    public class crc32
    {
        public static uint Hash(byte[] data, uint? crc = null, int? len = null)
        {
            ...
            uint res = 0xffffffff;
            if (crc != null) res = (uint)crc;
            //uint POLYNOMIAL = 0x82f63b78;
            int cnt = (len != null) ? (int)len : data.Length;
            for (int i = 0; i < cnt; i++)
            {
                res = crctab[((int)res ^ data[i]) & 0xff] ^ (res >> 8);
                //res ^= data[i];
                //for (int j = 0; j < 8; j++)
                //    res = (res >> 1) ^ (((res & 1) != 0) ? POLYNOMIAL : 0);
            }
        ...
        public static uint Hash(string file)
        {
            ...
            while (true)
            {
                ...
                if (readed <= 0) break;
                byte[] c = new byte[readed];
                res = Hash(c, res, readed);
            }
            fs.Close();
            fs.Dispose();
            return res;
        }
        public static JObject files(dFileCRCProcessing crcprocess, string path = ".", JObject res = null)
        {
            if (res == null) res = new JObject();
            string patt = Regex.Replace(path, @"\\", @"\\");
            patt = Regex.Replace(patt, @"\.", @"\.");
